...
-   `fa()` - calculates the flow anomaly, or the difference in flow for
    ...
    is wetter than the prior period.
...
-   `rate_of_change()` - is the estimate of the current instantaneous
    ...
    of the difference in streamflow at time T and T-1. Positive values
    mean streamflow is increasing and negative values mean streamflow is
    decreasing. This term can capture hysteresis effects in dissolved
    solutes and hydrograph limbs. This function estimates $\frac{d Q}{d t}$ 
    on the raw data or it will fit a cubic regression spline between date and 
    discharge when the argument `smooth = TRUE` is used and return the
